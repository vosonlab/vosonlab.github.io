...
The [VOSON](http://vosonlab.net/VOSON) software for hyperlink collection and analysis was an early research output of the VOSON Lab. It addressed a need for tools that could help study social connections online, even before the rise of social media, and assist researchers gain insights into important phenomena such as online social movements and networks around issue spheres [see @acklandoneil2011]. After [many years and many iterations](https://uberlink.com/voson-turns-10) since its inception in 2004, the canonical VOSON hyperlink collection software is now proud to re-introduce itself as part of our R open-source toolkit for social media collection `vosonSML`.
...
![Visualization of Hyperlink Network from previous VOSON v2.5 - features now available in vosonSML R package](voson_25.png){ style='width:837px !important;height:512px !important;' }
 
## Introduction

The vosonSML hyperlink collection and network creation works similarly to the 3-step process we use with other social media sources using the authenticate, collect and create verb functions. The `Authenticate` function is first called with the parameter "web" to identify and set up the context for subsequent operations, but does not require any further credentials in this implementation. We are using standard web crawling and text-based page scraping techniques to discover hyperlinks, and as such do not need to access any restricted data API's as we commonly do with social media.

### Installation

The new hyperlink collection and network features are currently only available in the [development version of vosonSML on github](https://github.com/vosonlab/vosonsml) but are to soon be released on CRAN. The development version can be installed as follows:
...
![Scope of hyperlinks collected using the max_depth parameter](crawl_depth.png)

As can be seen, a max depth of 1 directs the crawler to scrape and collect hyperlinks from only seed pages, a max depth of 2 to follow hyperlinks found on the seed pages and collect hyperlinks from those pages as well, and so on radiating outwards. The number of pages and hyperlinks can rise very rapidly so it's best to keep this number as low as possible. If a greater reach in collection sites is desired this could perhaps more efficiently be achieved by revising and adding more seed pages in the first instance. In the example code the `type` has been set to "ext" (external) for all three seed sites so as to limit "mapping" of the internal seed web sites and focus on their outward facing connections.
...
A total of 1163 hyperlinks were collected from 38 pages followed from our 3 seed pages. Using this data it's now possible to generate some hyperlink networks.
...
Now that the network has been generated we can create a graph and plot it. The `Graph` function creates an `igraph` format graph that can be directly plotted or adjusted for presentation using `igraph` plotting parameters.
...
![Hyperlink network of actors](hyperlink_network.png)
...
There is much more network visualization and analysis that could be performed on the `vosonSML` hyperlink networks and we will be working to add more features such as text analysis and network refinements in our near future releases. In the meantime we hope you've found this practical introduction to our new tool useful and look forward to your feedback!
