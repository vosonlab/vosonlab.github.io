...
The [VOSON](http://vosonlab.net/VOSON) software for hyperlink collection and analysis was an early research output of the VOSON Lab. It addressed a need for tools that could study social connections online, even before the rise of social media, and helped researchers gain insights into important phenomena such as online social movements and networks around issue spheres [see @acklandoneil2011]. After [many years and many iterations](https://uberlink.com/voson-turns-10) since its inception in 2004, the canonical VOSON hyperlink collection software is proud to re-introduce itself as part of our open-source toolkit for social media collection the `vosonSML` R package.

This simple guide will demonstrate how to use `vosonSML` to perform a hyperlink collection and generate networks for analysis.

![Visualization of Hyperlink Network from VOSON v2.5](voson_25.png){ style='width:512px !important;height:440px !important;' }
...
We now have a simple graph of the actor hyperlink network. Our seed actors are indicated by blue nodes and sites with an in-degree greater than one indicated in yellow. Node size and label size reflect most linked to nodes or highest in-degree. Perhaps unsurprisingly social media sites and the institutions at which the seed pages are located feature most prominently in the network and provides some context for each actors online presence and connections.

There is much more network visualization and analysis that could be performed on the `vosonSML` hyperlink networks. We hope you've found this practical introduction to our new tools useful!
