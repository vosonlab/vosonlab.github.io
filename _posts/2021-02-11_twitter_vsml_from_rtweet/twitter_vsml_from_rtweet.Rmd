...
auspol_tweets <- readRDS("./rtweet_tweets.rds")
```
...
The `rtweet` package has a method to save Twitter data to file in CSV format with the `write_as_csv` function.
...
If the data was saved to file with the `rtweet` function `write_as_csv` it can be read again using `read_twitter_csv`.
...
The resulting `igraph` network can then be plotted.
