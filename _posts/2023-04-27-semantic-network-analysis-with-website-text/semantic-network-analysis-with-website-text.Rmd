...
nrow(df2b)
# TO FIX: THE above two approaches should give the same dataframe, but they aren't...

#For time being, let's just use one of them
df2 <- df2a
...
g <- delete.vertices(g, which(V(g)$name=="NA"))       #remove NA node (problem with parsing)
...
We could visualise this in `gephi` and examine and interpret different clusters (identified using e.g. modularity clustering, which is available in `gephi`), but instead let's do this in R.

We will use the louvain cluster identification algorithm in `igraph` (same algorithm is used in `gephi`).

```{r}
set.seed(222)                     #set random seed (note: check if there is randomness in cluster generation)
mod <- cluster_louvain(g)          #community identification using modularity (same algorithm as in Gephi)
...
The above shows that there are 156 clusters identified, with the majority of these clusters only containing a small number of nodes.
