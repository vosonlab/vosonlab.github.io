...
### Identifying seed server instances
...
Data were collected in November 2023. We used the R package `rtoot` (REF) to programmatically access the list of ‘friends’ -- federated servers, with the `get_instance_peers` function -- and ‘foes’ -- moderated servers, with the `get_instance_blocks` function. 
...
| Neither | Otherwise | 296,129  | White | 
...
Then, we constructed a subnetwork including 'foe' ties only. This subnetwork encompasses the seeds which have sent negative ties and the nodes that have received at least one 'foe' nomination. The resulting network has 497 nodes and 643 'foe' edges. Isolates were removed. 

Nodes were colour coded based on the categorisation 
...
#+ 6/223032 edges from 288ae3e (vertex names):
#    [1] furry.engineer             ->iddqd.social social.veraciousnetwork.com->iddqd.social
#[3] vmst.io                    ->iddqd.social defcon.social              ->iddqd.social
#[5] social.linux.pizza         ->iddqd.social gametoots.de               ->iddqd.social

#Let's see the type of tie
e_ind[[which(V(g)$name=="iddqd.social")]]$type
#[1] "friend" "foe"    "foe"    "foe"    "foe"    "friend"
#So everthing looks correct: this node is "mixed" as there are two friendship nominations
#and 4 foe nominations


#other networks, just testing...

g3 <- induced.subgraph(g, which(V(g)$seed=="yes" |  V(g)$type %in% c("friend", "foe", "mixed")))

g3 <- induced.subgraph(g, which(V(g)$seed=="yes" |  V(g)$type %in% c("foe", "mixed")))
plot(g3, layout=layout_with_lgl(g3), vertex.size=3, vertex.label="", edge.arrow.size=0.3)


table(degree(g3))

V(g3)$color <- "white"
V(g3)$color[which(V(g3)$type=="friend")] <- "green"       #seed servers
V(g3)$color[which(V(g3)$type=="foe")] <- "red"       #seed servers
V(g3)$color[which(V(g3)$type=="mixed")] <- "orange"       #seed servers
V(g3)$color[which(V(g3)$seed=="yes")] <- "blue"       #seed servers
